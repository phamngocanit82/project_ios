...
#import "CTHFont.h"
#import "CTHHelper.h"
#import "CTCSlideIndicate.h"
@implementation CTCSlideIndicate
-(void)layoutSubviews{
    ...
    if(!self.trackImageView){
        ...
        NSInteger width = 14;
        NSInteger height = 80;
        CTCView *view = [[CTCView alloc] initWithFrame:CGRectMake(-width/2+self.distance, self.frame.size.height - width, width, width)];
        view.cornerRadius = width/2;
        view.drawBorder = YES;
        view.backgroundColor = [UIColor colorWithRed:0 green:130/255.f blue:101/255.f alpha:1];
        [self addSubview:view];
        
        view = [[CTCView alloc] initWithFrame:CGRectMake(self.frame.size.width - width/2-self.distance, self.frame.size.height - width, width, width)];
        view.cornerRadius = width/2;
        view.drawBorder = YES;
        view.backgroundColor = [UIColor colorWithRed:0 green:130/255.f blue:101/255.f alpha:1];
        [self addSubview:view];
        
        view = [[CTCView alloc] initWithFrame:CGRectMake(7+self.distance, self.frame.size.height - 8, self.frame.size.width-14-2*self.distance, 3)];
        view.backgroundColor = [UIColor colorWithRed:0 green:130/255.f blue:101/255.f alpha:1];
        [self addSubview:view];
        
        width = 40;
        self.trackView = [[CTCView alloc] initWithFrame:CGRectMake(-width/2+self.distance, self.frame.size.height - height, width, height)];
        self.trackImageView = [[CTCImageView alloc] initWithFrame:CGRectMake(0, 0, width, height)];
        self.trackImageView.image = [UIImage imageNamed:@"ic_track"];
        [self.trackView addSubview:self.trackImageView];
        
        self.valueLabel = [[CTCLabel alloc] initWithFrame:CGRectMake(0, 0, width, width)];
