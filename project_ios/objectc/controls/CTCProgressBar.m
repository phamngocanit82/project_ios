...
#import "CTCProgressBar.h"
@interface CTCProgressBar (){
@private
    CTCView *progressBar;
    CTCLabel *percentLabel;
}
@end
@implementation CTCProgressBar
-(void)layoutSubviews{
...
-(void)drawBackground{
    if(!progressBar){
        CTCView *background = [[CTCView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height)];
        ...
        progressBar = [[CTCView alloc] initWithFrame:CGRectMake(2, 2, 0, self.frame.size.height-4)];
        ...
        percentLabel = [[CTCLabel alloc] initWithFrame:CGRectMake(progressBar.frame.size.width-55, 0, 50, self.frame.size.height-5)];
        ...
        //[CTCProgressBar addSubview:percentLabel];
