#import "CTCView.h"
#import "CTHHelper.h"
#import "CTCDepthScrollView.h"
@implementation CTCDepthScrollView
-(void)awakeFromNib{
    [super awakeFromNib];
    self.delegate = self;
    angleRatio = 0.3;
...
-(void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    ...
    for(UIView *view in self.subviews){
        if((view.tag-200)!=self.currentPage){
            CTCView *darkView = (CTCView*)[CTHHelper controlFromTag:20 withView:view kind:[CTCView class]];
            //if(darkView != nil)
                //darkView.alpha = 0.4;
        }else{
            currentView = (CTCView*)[CTHHelper controlFromTag:20 withView:view kind:[CTCView class]];
        }
    }
    if(currentView!= nil){
        //currentView.alpha = 0;
